...
const PAGE_STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  /* Unlike the <img>, an embedded prototype has no intrinsic size to lay
     out from, so it's given an explicit box matching .carousel-viewport's
     own min-height instead of sizing to the frame's design width. */
  .carousel-slide iframe { display:block; width:100%; height:max(320px, 76vh); border:0; }
...
// Figma's embeddable prototype player. node-id wants the frame key with
// dashes; starting-point-node-id wants it with colons (URLSearchParams
// percent-encodes that for us) so playback opens on this specific frame
// rather than wherever the file's own default prototype start point is.
// embed-host is a required, arbitrary identifier for who's embedding it.
function buildPrototypeEmbedUrl(fileKey, frameKey) {
  const params = new URLSearchParams({
    "node-id": frameKey.replace(/:/g, "-"),
    "starting-point-node-id": frameKey,
    "embed-host": "off-you-pop",
    "hide-ui": "1",
  });
...
//
// Before signing, a frame with a recorded figma_file_key gets a live,
// interactive Figma prototype embed instead of the static snapshot image —
// so a client can click through prototyped components the way they would
// in Figma's own Present mode. Once signed, every recipient permanently
// sees the frozen snapshot image instead, forever, even on repeat visits —
// the signed record has to keep meaning "this exact image was approved",
// and a live embed doesn't hold still. Records saved before this feature
// existed have no figma_file_key, so they just keep using the image.
function renderSnapshots(snapshots, { isSigned, figmaFileKey }) {
  return snapshots
    .map((s, i) => {
      const embedUrl =
        !isSigned && figmaFileKey && s.figma_frame_key
          ? buildPrototypeEmbedUrl(figmaFileKey, s.figma_frame_key)
          : null;
      return html`
        <div class="carousel-slide${i === 0 ? " is-active" : ""}" data-index="${i}">
          ${embedUrl
            ? html`<iframe
                ...
                allowfullscreen
                loading="lazy"
                title="${s.figma_node_name || "Frame"} — interactive preview"
              ></iframe>`
            : html`<img
                src="${s.snapshot_url}"
  ...
  return raw(`
    (function () {
      ...
      function showSlide(index) {
        ...
        // A frame can be much taller than the viewport (a full-page
        // mockup) — without this, flicking to the next one leaves you
        // wherever you'd scrolled to on the previous frame instead of at
        // its top.
...
<html>
  ...
  <body>
    ...
    <div class="stage">
      <div class="carousel-viewport" id="frame-carousel">${raw(renderSnapshots(snapshots, { isSigned, figmaFileKey: record.figma_file_key }))}</div>
    </div>
