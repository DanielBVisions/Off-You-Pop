...
const PAGE_STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  /* The stage: a full-bleed, near-full-viewport-height dark canvas right
     below the bar, modeled on Figma's own Present mode. The image is
     capped at its own native pixel width (width:auto — a plain <img> with
     no explicit width renders at its intrinsic size) and only ever
     shrinks to fit a narrower viewport (max-width:100%), never stretches
     past its real resolution — stretching a 1440px-wide export to fill a
     wider browser window was what was actually causing the blur, at any
     export scale, since it upscales past the image's real pixel density.
     Centered via margin:auto when the stage is wider than the frame
     itself, matching Figma's own letterboxed-on-a-dark-canvas look, and
