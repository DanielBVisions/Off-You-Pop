...
function sendHtml(res, status, html, extraHeaders = {}) {
  ...
  // lib/html.js's `html` tagged template returns a raw()-wrapped object,
  // not a plain string (that's what makes nested html`` calls compose
  // safely) — but Node's res.end() throws for anything that isn't
  // actually a string/Buffer, it does not call toString() itself.
  res.end(String(html));
}
