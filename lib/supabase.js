...
async function parseResponse(res) {
  ...
  if (!res.ok) {
    const knownField =
      body && typeof body === "object" ? body.message || body.error_description || body.error || body.msg : null;
    // Always include the raw body, not just when none of the known field
    // names match — Supabase's error shape varies by endpoint/version,
    // and a message that silently falls back to a generic string hides
    // exactly the detail needed to debug (see: two rounds of that
    // happening already on this same login flow).
    const rawBody = typeof body === "string" ? body : JSON.stringify(body);
    const message = knownField
      ? `${knownField} (${res.status})`
      : `Supabase request failed (${res.status}): ${rawBody}`;
...
/** `query` must be non-empty — this is a real DELETE, no accidental whole-table wipes. */
async function pgDelete(table, query) {
  if (!query.length) throw new Error("pgDelete requires at least one filter");
  const res = await fetch(`${baseUrl()}/rest/v1/${table}?${query.join("&")}`, {
    method: "DELETE",
